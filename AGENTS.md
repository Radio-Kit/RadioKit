...
- `fs`: Remote GitHub repository or subfolder link used by the app's Filesystem Tab repository browser. Written to NVS as `rk_fs_url` on first boot.
- `ota`: Remote OTA firmware update link placeholder. Written to NVS as `rk_ota_url` on first boot.
