...
struct RK_ButtonFields {
    ...
    int16_t     rotation;
    const char* icon;       ///< ON state icon (or default icon)
    const char* offIcon;    ///< OFF state icon (optional override)
    const char* onText;
    const char* offText;
    const char* label;
...
| Set icon (ON state) | `btn.rk.icon = "bell-ringing";` |
| Set offIcon (OFF state) | `btn.rk.offIcon = "bell";` |
...
// Toggle button with dual icons
RK_ToggleButton power(20, 80, 20);
power.rk.label   = "Power";
power.rk.icon    = "power";
power.rk.offIcon = "power-off";
...
struct RK_MultipleFields {
    ...
    uint8_t     itemMask;         ///< Item visibility bitmap (bit i: 1=visible, 0=hidden)
...
| Set item visibility mask | `multi.setItemMask(0x0B);` |
| Hide / show item | `multi.setItemHidden(2, true);` / `multi.setItemVisible(0, true);` |
| Query item visibility | `bool visible = multi.isItemVisible(0);` |
...
toolbar.setItemMask(0b00000111); // All 3 items visible
...
struct RK_SlideSwitchFields {
    ...
    uint8_t     height, width;
    int16_t     rotation;
    const char* icon;       ///< ON state icon
    const char* offIcon;    ///< OFF state icon
    const char* onText;
    const char* offText;
    const char* label;
...
| Set icons | `sw.rk.icon = "arrow-right"; sw.rk.offIcon = "arrow-left";` |
...
6. **Max 32 widgets** — Plan your UI to fit within `RADIOKIT_MAX_WIDGETS`.
