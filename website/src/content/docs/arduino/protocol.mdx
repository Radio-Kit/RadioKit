...
| `0x8F` | `LINKS_INFO_DATA` | `[FS_LEN][FS_URL...][OTA_LEN][OTA_URL...]` | Remote links payload (streamed directly from `RadioKit.config.fs_url` and `RadioKit.config.ota_url`) |
...
Over-the-air firmware updates using ESP32's `Update` library. OTA update support is activated at runtime by calling `RadioKit.enableOTA()` in the sketch, which dynamically sets the `RK_SETTINGS_FEATURE_OTA` bit in `FEATURES_DATA`.
